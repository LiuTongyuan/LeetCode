 ...
 * @Description
 */
public class Num2192_getAncestors {
        ...
        for (int i = 0; i < n; i++) {
            if (inD[i] == 0) {
                ...
                while (!queue.isEmpty()) {
                    ...
                    List<Integer> curTo = to.get(cur);
                    for (Integer next : curTo) {
                        if (!res.get(next).contains(cur)) {
                            res.get(next).add(cur);
                        }
                        for (Integer nu : res.get(cur)) {
                            if (!res.get(next).contains(nu)) {
                                res.get(next).add(nu);
                            }
                        }
                    }
    ...
    public static List<List<Integer>> getAncestors2(int n, int[][] edges) {
        ...
        while (!deque.isEmpty()) {
            ...
            for (Integer now : to.get(cur)) {
                inD[now]--;
                if (inD[now] == 0) {
                    deque.addLast(now);
                }

                pre[now][cur / 32] |= 1 << (cur % 32);
                for (int j = 0; j < pre[now].length; j++) {
                    pre[now][j] |= pre[cur][j];
                }
            }
        }
        ...
        for (int i = 0; i < pre.length; i++) {
            ...
            for (int j = 0; j < pre[i].length; j++) {
                int last = pre[i][j] - ((pre[i][j] - 1) & pre[i][j]);
                while (last != 0) {
                    pre[i][j] -= last;
                    res.get(i).add(32 * j + countZero(last));
                    last = pre[i][j] - ((pre[i][j] - 1) & pre[i][j]);
                }
            }
        }
